...
function aiExecuteWorkaround_PARAM_ID() {
    ...
    if (window!=window.top) { /* I'm in a frame! */ 
      ...
      if (updateIframeHeight == 'true') {      
        ...
        var url = domain_PARAM_ID + domain_PARAM_ID_path + '/js/iframe_height.html';
        ...
        if (iframe_PARAM_ID_last_height != newHeight) { // we only resize if we have a change
            ...
            if (onload_resize_delay == 0) {
               ...
               setTimeout(function () { resizeLater_PARAM_ID(iframe); }, onload_resize_delay);
            }
        ...
        // add the iframe dynamically
        var url = domain_PARAM_ID + domain_PARAM_ID_path + '/js/iframe_show.html?id='+ iframe_id_PARAM_ID;
...
function resizeLater_PARAM_ID(iframe_obj) {
   var url = domain_PARAM_ID + domain_PARAM_ID_path + '/js/iframe_height.html';
   var wrapperElement = aiGetWrapperElement(element_to_measure);
   var newHeightRaw =  Math.max(wrapperElement.scrollHeight, wrapperElement.offsetHeight);
...
var domain_PARAM_ID_path = '/wp-content/plugins/advanced-iframe'; // Check if this is the path to the main directory of aip

// load an external config from the inital site. Param config_id is checked from the url. 
var doIt = loadExternalConfig(domain_PARAM_ID + domain_PARAM_ID_path);
...
if (doIt) {
  ...
  if (resize_on_element_resize != '') {
      document.write("<script src='PARAM_RESIZE_PATH'></script>");
  }
