...
<div>
    ...
    <p><?php _e('The following additional files in the folder "advanced-iframe-custom" do currently exist. Please note that you can view/edit this files with the plugin editor of Wordpress by clicking on the "Edit/View" link. Hover over the file you want to include and click 3 times fast to select it.', 'advanced-iframe') ?> 
    </p>
<?php
  $config_files = array();
...
if (count($config_files) == 0) {
  ...
  foreach ($config_files as $file) {
    ...
    echo '</a></div>';
    echo '<div class="ai-external-config">' . site_url() . '/wp-content/plugins/advanced-iframe-custom/'.$file.'</div>';
    echo '<br />';
  }
}
echo "<hr height=1>";
?>  
    <p><?php _e('Create a custom file. Only files with the extensions css or js are allowed. All files are prefiex with "custom_":', 'advanced-iframe') ?><br /><input name="ai_custom_id" id="ai_custom_id" type="text" size="20" maxlength="20" /> 
      <input id="ccf" class="button-primary" type="submit" name="create-custom-id" value="<?php _e('Create custom file', 'advanced-iframe') ?>"/>
    </p>
    </div>
     
    <h2><?php _e('Parent', 'advanced-iframe') ?></h2>
    <p>
...
<?php
    ...
    <?php if ($evanto) { ?> 
    <h2><?php _e('Iframe', 'advanced-iframe') ?></h2>
    <p>
