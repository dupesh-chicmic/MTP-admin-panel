...
';
	...
	';													else if(in_array($nf_functions->get_ext(trim($item)),$file_ext_array))
					...
					foreach($_POST['selection'] as $key=>$val)
						{
						$get_form_entry = $wpdb->prepare('SELECT form_data,title FROM '.$wpdb->prefix.'wap_nex_forms_entries, '.$wpdb->prefix.'wap_nex_forms WHERE '.$wpdb->prefix.'wap_nex_forms_entries.nex_forms_Id = '.$wpdb->prefix.'wap_nex_forms.Id AND '.$wpdb->prefix.'wap_nex_forms_entries.Id='.$val);
								...
								$output .= '<div class="row batch_entry" id="'.$val.'">';
									$output .= '<div class="col-sm-12">';
										...
										$output .= '<div class="form_data">';
											...
											foreach($form_data as $data)
												{
												$set_data .= $nf_functions->unformat_records_name($data['field_name']).': '.$data['field_value'].' | ';
												}
									
											$output .= $nf_functions->view_excerpt(str_replace('\\','',$set_data),400);
										$output .= '</div>';
...
function NEXForms_entry_status($form_Id){
			...
			$entry = $wpdb->get_var('SELECT viewed FROM '.$wpdb->prefix.'wap_nex_forms_entries WHERE Id='.$set_form_id);
...
function NEXForms_starred($form_Id){
			...
			if(is_array($form_Id))
				$set_form_id = $form_Id[0];
			$entry = $wpdb->get_var('SELECT starred FROM '.$wpdb->prefix.'wap_nex_forms_entries WHERE Id='.$set_form_id);
...
function NEXForms_get_attachment($form_Id){
			...
			if(is_array($form_Id))
				$set_form_id = $form_Id[0];
			$entry = $wpdb->get_var('SELECT * FROM '.$wpdb->prefix.'wap_nex_forms_files WHERE entry_Id='.$set_form_id);
...
function NEXForms_get_title3($Id='',$table=''){
			...
			return '<span class="the_form_title" data-form-id="'.$Id.'">'.str_replace('\\','',$the_title).'</span>';
		}
