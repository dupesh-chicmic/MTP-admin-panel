...
function NEXForms_dashboard(){
	...
	$saved_forms->extra_buttons = array('new_form'=>array('class'=>'create_new_form', 'id'=>isset($_POST['form_Id']) ? $_POST['form_Id'] : '', 'type'=>'button','link'=>'', 'icon'=>'<span class="fas fa-file-medical"></span> '.__('&nbsp;&nbsp;Add a New Form','nex-forms').''));
...
if(!class_exists('NEXForms_dashboard'))
	...
	class NEXForms_dashboard{
		...
		public function __construct($table='', $table_header='', $extra_classes='', $table_header_icon='',$additional_params='', $search_params='', $table_headings='', $show_headings='', $field_selection ='', $extra_buttons ='', $checkout=false, $sortable_columns = true, $show_search=true, $show_paging=true, $show_delete=false, $is_report=false, $color_adapt=false, $table_resize=false , $record_limit=10, $action=''){
			...
			$entries = $wpdb->get_results('SELECT nex_forms_Id, COUNT(nex_forms_Id) as counted FROM `'.$wpdb->prefix.'wap_nex_forms_entries` WHERE trashed IS NULL GROUP BY nex_forms_Id;');
		...
		public function dashboard_menu($page_title){
				...
				$set_folder = isset($_REQUEST['folder']) ? $_REQUEST['folder'] : 0;
					...
					$output .= '<div class="nav-container prime-menu">';
						...
						$output .= '<div class="inner">';
							$output .= '<ul class="navigation aa_menu">';
								$output .= '<li class="logo">NEX-Forms </li>';//
		...
		public function print_record_table(){
			...
			$show_delete = (isset($_POST['show_delete'])) ? $_POST['show_delete'] : $this->show_delete;
		...
		public function get_table_records($additional_params=array(), $search_params=array(), $header_params=array(), $is_report=false){
			...
			$do_action = (isset($_POST['do_action'])) ? $_POST['do_action'] : $this->action;
			...
			$sort_by = (isset($_POST['sort_by']) && $_POST['sort_by']!='') ? $_POST['sort_by'] : 'Id';
			
			
			$sort_by = $sort_by_table.$sort_by;
			
			$sort_by_direction =(isset($_POST['sort_by_direction']) && $_POST['sort_by_direction']!='') ? $_POST['sort_by_direction'] : 'DESC';
			
			$record_limit = (isset($_POST['record_limit'])) ? $_POST['record_limit'] : $this->record_limit;
			...
			$is_report = (isset($_POST['is_report'])) ? $_POST['is_report'] : $this->is_report;
	...
	public function submission_report(){
			...
			//$output .= '<div class="right-bottom">';
				/*$output .= '<div class="row row_zero_margin aa_bg_tri reporting_controls">';
				
				$show_cols = $_POST['showhide_fields'];
				
				$output .= '<div class="col-sm-3 field_selection_col make_sticky">';
				...
				//else
				 ...
				 foreach($col_array_unique as $key=>$val)
					...
					$output .= '</select></div>';
				 $output .= '<div class="col-sm-1 make_sticky">';
						$output .= '<a class="btn add_new_where_clause"><i class="fa fa-plus"></i> </a>';	
					 $output .= '</div>';
				
				$output .= '<div class="col-sm-8 clause_container zero_padding">';
			 ...
			 $output .= '<div class="right-col-top">'; 
				...
				//else
				 ...
				 $output .= '<div class="col-sm-1 add_clause">';
						...
						$output .= '<a class="nf_button aa_bg_main_btn run_query_2 run_query" id="'.$_POST['form_Id'].'"><i class="fa fa-file-import"></i> Run Query </a>';
