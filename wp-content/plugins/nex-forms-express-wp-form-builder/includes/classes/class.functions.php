	...
	if(!empty($rules))
			...
			foreach($rules as $rule)
				...
				if($rule)
					...
					if( $condition->field_type=='checkbox' && $rule_operator=='any')
						{
						$output .= 'var action_targets 		= [];';	
						$output .= 'var action_arrows 		= [];
						
						action_arrows.push("'.$condition->condition_value.'");
						';
						...
						if($con_field_type)
							...
							if($get_the_condition=='contains' || $get_the_condition=='not_contains')
								...
								if($con_field_type=='select')
									...
									{
									$output .= $set_bool.'strstr(parseFloat(nf_str_to_lower(jQuery(\'#nf_form_'.$unigue_form_Id.' #'.$con_field_id.'\').find(\'input[type="'.$con_field_type.'"]'.$add_string.'\').val())), '.$set_the_value.')';
									}
								else
									{
								...
								if($con_field_type=='select')
									...
									{
									$output .= 'parseFloat(nf_str_to_lower(jQuery(\'#nf_form_'.$unigue_form_Id.' #'.$con_field_id.'\').find(\'input[type="'.$con_field_type.'"]'.$add_string.'\').val()))'.$set_operator.''.$set_the_value.' '.(($i<($query_length-1)) ? $if_clause : '' );
									}
								else
									{
									$output .= 'nf_str_to_lower(jQuery(\'#nf_form_'.$unigue_form_Id.' #'.$con_field_id.'\').find(\'input[type="'.$con_field_type.'"]'.$add_string.'\').val())'.$set_operator.''.$set_the_value.' '.(($i<($query_length-1)) ? $if_clause : '' );
					...
					foreach($rule->actions as $action)
						...
						$action_field_type	 = $action->target_field_type;
						$action_field_name	 = $action->target_field_name;
						$action_to_value	 = (isset($action->change_value)) ? '"'.$action->change_value.'"' : '';
		...
		jQuery(document).ready(
				function()
					...
					jQuery(document).on(\'change\', \'#nex-forms input , #nex-forms select, #nex-forms textarea\',
						function()
							...
							if(jQuery(this).is(":checkbox")){
							  setTimeout(function(){ run_nf_conditional_logic'.$function_post_fix.'(the_obj); }, 350);
							}
							else
								setTimeout(function(){ run_nf_conditional_logic'.$function_post_fix.'(jQuery(this)); }, 350);
							}
						);
					jQuery(document).on(\'keyup\', \'#nex-forms input, #nex-forms textarea\',
						function()
					...
					setTimeout(function(){ run_nf_conditional_logic'.$function_post_fix.'("")}, 350);
