...
function NEXForms5_run_instalation(){
	...
	add_option('nex-forms-preferences',
		array(
			'field_preferences'=>
				array(
...
function NEXForms_ui_output( $atts , $echo='',$prefill_array='',$unigue_form_Id=''){
				...
				$output .= '<div data-timer-start="'.$timer_start.'" data-timer-end="'.$timer_end.'" class="inner-canvas-container ui-nex-forms-container '.(($add_timer=='yes') ? 'has_time_limit timer_'.$timer_type : '' ).' '.(($form_attr->form_theme) ? $form_attr->form_theme : 'bootstrap').'" id="ui-nex-forms-container"  >';
					...
					if($is_paypal=='yes' && function_exists('nf_not_found_notice_ppp'))
						...
						$output .= '<div class="current_field_on_focus" style="display:none">1</div>';
							...
							//<FORM> Start
							$output .= 	'<form id="nf_form_'.$unigue_form_Id.'" class="'.$set_ajax.'" name="nex_form" action="'.$set_post_action.'" method="'.$post_method.'" enctype="multipart/form-data">';
								//Step Transitions
	...
	if($styles_config['incstyle-animations']=='1')
	...
	//preg_match_all("/([\\d.]+)/", $output, $rgb_matches);
...
function submit_nex_form(){
	...
	$nex_forms_id = isset($_REQUEST['nex_forms_Id']) ? sanitize_text_field($_POST['nex_forms_Id']) : '';
...
/**************************************************/
	...
	$ftp_body = (($_POST[$ftp_field_map['post_content']]) ? sanitize_text_field($_POST[$ftp_field_map['post_content']]) : sanitize_text_field($ftp_field_map['post_content']));
	...
	preg_match_all($pattern, $ftp_body, $matches2);
			...
			$set_ftp_array = array(	
						...
						'post_title'		=>	sanitize_text_field($_POST[$ftp_field_map['post_title']]),
					...
					$output .= '<div class="add_on_item">';	
						...
						$output .= '<div class="add_on_check">';	
							...
							if(function_exists('NEXForms_not_found_notice_zapier'))
								...
								{
								$output .= '<a href="https://codecanyon.net/item/zapier-integration-for-nexforms/38000632?ref=Basix" class="buy_add_on btn btn-lime" target="_blank">Get Add-on</a>';	
								}
