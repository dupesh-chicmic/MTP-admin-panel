...
foreach($modelImport as $item){
    ...
    echo '<tr style="'.$backgroundColor.' border-bottom:1px dashed #CCCCCC">';
        ...
        echo '<td style="padding-right:50px; text-align:center;">';
            echo '<span style="color:#2D8296; font-weight:bolder;"><span class="actionLink_text"><a href="'.(($carsVisibleLink) ? Yii::app()->createUrl('member/usedCarsArchive',array('arch'=>$item->id)) : '#').'">'.$item->data.' <text style="'.(($carsVisibleLink) ? 'color:#2D8296;' : 'color:#CCC;').'">Used Cars '.$item->nazwa.'</text></span></a></span></span>';
        echo '</td>';
    //echo '<br />';
    if(!empty($item->usedComCarsCount)){
        echo '<td style="padding-left:50px; text-align:center;">';
            echo '<span style="color:#2D8296; font-weight:bolder;"><span class="actionLink_text"><a href="'.(($commVisibleLink) ? Yii::app()->createUrl('member/usedComCarsArchive',array('arch'=>$item->id)) : '#').'">'.$item->data.' <text style="'.(($commVisibleLink) ? 'color:#2D8296;' : 'color:#CCC;').'">Used Commercial '.$item->nazwa.'</text></span></a></span></span>';
        echo '</td>';
    }
