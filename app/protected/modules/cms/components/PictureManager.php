...
class PictureManager{
    ...
    public function createMainAndSubFolders($path, $subFolder1, $subFolder2, $subFolder3=null){
        /* tworzy glowny folder oraz np. m,s,d dla galerii */
        try{
            $mainDirectoy = mkdir($path, 0777);
            if($mainDirectoy == '1'){
    ...
    public function addPicureInMainFolder($model, $path, $picWidth, $picHeight, $formPictureField='image'){
        try{
            $skipDeletingOldFile = false;
            
            if(!file_exists($path)){
                $mainDirectoy = mkdir($path, 0777);
                $skipDeletingOldFile = true;
            }
                ...
                $fileName = time().'_thumb.'.$file_extension;
                        ...
                        if($type == 'image/jpeg' || $type == 'image/png' || $type == 'image/gif' || $type == 'image/pjpeg' || $type == 'image/x-png'){
                            ...
                            if(file_exists($path)){//echo 'flash';
                                ...
                                if ($oldDefaultPic != '' && !$skipDeletingOldFile){
    ...
    public function addPicureInSubFolders($model, $MainPath, $foldersList, $formPictureField='image'){
        try{
            ...
            $mainDirectoy = mkdir($MainPath, 0777);
            
                if($mainDirectoy == '1'){
                    foreach($foldersList as $folders_key=>$folders_val){
                        mkdir($MainPath.'/'.$folders_key, 0777);                
                    }
                }
                $skipDeletingOldFile = true;
            }
            ...
            if(empty($image)){ return NO_IMAGE; }
                ...
                $fileName = time().'_thumb.'.$file_extension;
                        ...
                        if($type == 'image/jpeg' || $type == 'image/png' || $type == 'image/gif' || $type == 'image/pjpeg' || $type == 'image/x-png'){
                            ...
                            }else{ //echo "Main folder doesn't exist"; 
                                return NO_IMAGE;
                            }
                ...
                $fileName = time().'_thumb.'.$file_extension;
                        ...
                        if($type == 'image/jpeg' || $type == 'image/png' || $type == 'image/gif' || $type == 'image/pjpeg' || $type == 'image/x-png'){             
                            if(file_exists($path)){
                                ...
                                if ($oldDefaultPic != '' && !$skipDeletingOldFile&&  file_exists($oldDefaultPicDel)){
                                    unlink($oldDefaultPicDel);    
                                }
                                return $imageName;
                            }else{ echo "Main folder doesn't exist"; return NO_IMAGE; }
...
/* --- */    
    ...
    public function deleteFolderOrFile($fullPathWithFileName){
        if (file_exists($fullPathWithFileName)){                                
            ...
            echo "File doesn't exist"; return ERROR_DELETE;
        }        
    }
    ...
    public function ImgResize($width, $height, $path, $folder=null){        
        ...
        if($folder == 'm'){
                    $fileName = Yii::app()->getModulePath().ImageHelper::thumb($width,$height,$path, array(
                        'method' => 'resize',
                        'quality' => 90));            
        }else if($folder == 'd'){
                    $fileName = Yii::app()->getModulePath().ImageHelper::thumb($width,$height,$path, array(
