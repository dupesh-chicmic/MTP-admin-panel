...
class ImageHelper {
    ...
    public static function thumb($width, $height, $img, $options = null)
    {
        ...
        //$pathinfo['filename'] = time();
        $pathinfo['filename'] = substr($pathinfo['filename'], 0 , 10); //pokaz pierwsze 10 znakow reszta w kosz
        ...
        if(!file_exists($thumb_path.$thumb_name) || filemtime($thumb_path.$thumb_name) < filemtime($img)){
            ...
            //echo Yii::app()->getModulePath();
            //echo dirname(__FILE__);
            Yii::setPathOfAlias('phpTumbModuleCms', Yii::app()->getModulePath().'/cms');
            Yii::import('phpTumbModuleCms.extensions.phpThumb.PhpThumbFactory');
